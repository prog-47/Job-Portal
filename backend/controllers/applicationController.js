...
export const getMyApplications = async(req,res) =>{
    try{
        const apps = await Application.find({ applicants : req.user._id })
...
export const getApplicantsForJob = async(req,res) => {
    try{
      ...
      const application = await Application.find({ job: req.params.jobId })
       .populate("job", "title location category avatar resume ")
        .populate("applicants", "name email avatar resume");
...
//@desc Get all application by ID (Jobseeker or Employer)
export const getApplicationById = async (req,res) => {
     try{
        const app = await Application.find(req.params.id)
         .populate("job", "title")
         .populate("applicants", "name email avatar resume");
